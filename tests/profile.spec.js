...
let profileData = require("../testData/profileData.json")
...
test('TC01_PrfileUpdate', async ({page})=>
{
    ...
    let username = await page.getByPlaceholder('Enter your active Email ID / Username').fill(profileData.userName);
    let password = await page.getByPlaceholder('Enter your password').fill(profileData.passWord);
    ...
    // =========================== Uploading resume ===========================
    await page.locator('//input[@id="attachCV"]').setInputFiles('testData/QA_Automation_Resume.pdf');
    ...
    console.log('\n=====================================')
    let searchAppearance = await page.locator("//div[text()='Search Appearances']/parent::div/child::div[@class='nI-gNb-de__perf-card-count']").textContent();
    let recruiterActions = await page.locator("//div[text()='Recruiter Actions']/parent::div/child::div[@class='nI-gNb-de__perf-card-count']").textContent();
    console.log(`Search Appearance : ${searchAppearance}`);
    console.log(`Recruiter Actions : ${recruiterActions}`);
    console.log('=====================================')
